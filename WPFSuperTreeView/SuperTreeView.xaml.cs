    ...
    public partial class SuperTreeView : UserControl
    {
        ...
        public String EFConnectionString
        {
            ...
            set
            {
                ...
                _repository = new NodeTreeRepository(_EFConnectionString);
              
            }
        }
        private String _treeNodeType="";
        ...
        /// 用于存取数据库
        /// </summary>
        private NodeTreeRepository _repository = null;
          ...
          //  treeNode.Icon = nodeDataObject.DataItem.NormalIcon;
            ...
            switch (addType)
            {
                case AddNodeCategory.AddRoot:
                    ...
                    nodeDataObject.DataItem.NodeText = nodeText;
                ...
                case AddNodeCategory.AddSibling:
                    if (selectedNode != null && selectedNode.Parent != null)
                    {
                        ...
                        if (selectedNode.Parent == tree)
                        {
                            treeNode.Path = "/" + nodeText + "/";
                            nodeDataObject.DataItem.Path = treeNode.Path;
                         //   treeNode.IconType = nodeDataObject.DataItem.IconType;
                            nodeDataObject.DataItem.NodeText = nodeText;
                            tree.Items.Add(treeNode);
                        }
                        else
            ...
            if (_repository != null)
            {
                _repository.SaveTree(treeXml, _treeNodeType);
            }
        ...
        private void InnerTreeSelectedItemChanged(object sender, RoutedPropertyChangedEventArgs<object> e)
        {
            ...
            if (SelectedItemChanged2 != null && ShouldRaiseSelectedItemChangedEvent)
            {
                SelectedItemChanged2(sender, e);
            }


        }
        /// <summary>
        /// 用于将内部TreeView的SelectedItemChanged事件导出到UserControl外部以方便使用。只有继承了ItemControl的控件
        /// 才具有SelectedItemChanged事件。其实在TreeView中有SelectedItemChanged事件公有定义但却增加了特性表示，
        /// 所以此处必须重新定义才能使用，为了方便可以改为SelectedItemChanged2以作区别。由于本控件会嵌入到其他控件中，所以
        /// 相当于SelectedItemChanged2 可以在外部控件中使用。
        /// 于是当本控件SelectedItemChanged事件发生时，可以通过上述处理函数路由通知到本控件的SelectedItemChanged2 事件
        /// 从而通知到外部控件。
        /// </summary>
        public event RoutedPropertyChangedEventHandler<object> SelectedItemChanged2;
        ...
        #region "XML序列化支持"
        #region "将树视图中的全部节点信息转化为XML树表示，然后将XML树序列化为内存流，将得到的内存流转化为字符串，从而方便保存到数据库中。"
        /// <summary>
        /// 根据某一节点创建对应的Xml元素
        /// </summary>
        /// <param name="node">某一节点</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 根据树视图第一层节点中的某一节点创建其所有子节点对应成的Xml树
        /// </summary>
        /// <param name="firstLevelNode">第一层树视图节点</param>
        /// <param name="firstLevelElement">第一层XML节点</param>
        private void CreateXmlTreeFromTreeViewIconsItem(TreeViewIconsItem firstLevelNode, XElement firstLevelElement)
        {
            XElement tempElement = null;

            foreach (var item in firstLevelNode.Items)
            {  
                tempElement = CreateXElementFromTreeViewIconsItem(item as TreeViewIconsItem);
                firstLevelElement.Add(tempElement);
        ...
        /// <summary>
        /// 提取TreeView中的所有节点，创建整个XML树
        /// </summary>
        /// <returns></returns>
        private XElement CreateXmlTree()
        {
            XElement rootElement = new XElement("Root");
            XElement firstLevelNode = null;//第一层的XML节点
        ...
        public void SaveToDB()
        {
            if (_repository != null)
            {
                _repository.SaveTree(saveToXmlString(), _treeNodeType);
            }
        }
        /// <summary>
        ...
        #endregion
        #region "从数据库中获得用XML表示的字符串，将字符串转化为节点树，"
        /// <summary>
        /// 从数据库中装载树
        /// </summary>
        public String LoadTreeXMLFromDB()
        {
            ...
            return _repository.GetTreeFromDB(_treeNodeType);
        }
        /// <summary>
        /// 从Xml文件中装入树
        /// </summary>
        ...
        private TreeViewIconsItem CreateTreeViewNodeFromXElement(XElement element)
        {
            ...
            TreeViewIconsItem node = new TreeViewIconsItem(this, nodeDataObject, element.Attribute("IconType").Value);
        ...
        private void ClickToReName(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
           ...
           if (e.OriginalSource is TextBlock)
            {
                ...
                TreeViewIconsItem selectedNode = SelectedItem as TreeViewIconsItem;
                if (selectedNode != null)
                {
